...
class AccountCheck(models.Model):
    ...
    type = fields.Selection(
        [('issue_check', 'Issue Check'), ('third_check', 'Third Check'),
         ('issue_promissory_note', 'Issue Promissory Note'), ('deposit_promissory_note', 'Deposit Promissory Note')],
        readonly=True,
        index=True,
    )
    partner_id = fields.Many2one(
    ...
    def _compute_first_partner(self):
        for rec in self:
            rec.first_partner_id = rec.operation_ids and rec.operation_ids[0].partner_id or False
    ...
    def _check_state_change(self, operation):
        ...
        * value is 'from states'
        ...
        operation_from_state_map = {
            # 'draft': [False],
            'holding': [
                'draft', 'deposited', 'sold', 'delivered', 'transfered'],
            'delivered': ['holding'],
            ...
            'transfered': ['holding'],
            'withdrawed': ['draft'],
            'rejected': ['delivered', 'deposited', 'sold', 'handed'],
            'debited': ['handed'],
...
# checks operations from checks
    ...
    def reject(self):
        self.ensure_one()
        if self.state in ['deposited', 'sold']:
...
#         }
        ...
        if operation in ['rejected', 'reclaimed'] and self.type == 'third_check':
            name = 'Rejected check "%s"' % (self.name)
        elif operation == 'returned' and self.type == 'third_check':
            name = 'Returned check "%s"' % (self.name)
#         elif operation in ['rejected', 'reclaimed'] and self.type == 'deposit_promissory_note':
#             name = 'Rejected  promissory note "%s"' % (self.name)
#         elif operation == 'returned' and self.type == 'third_promissory_note':
#             name = 'Returned promissory note "%s"' % (self.name)
        else:
            raise ValidationError(_(
                'Debit note for operation %s not implemented!' % (
                    operation)))

        vals = self.prepare_new_operation_move_values(debit_account, credit_account, name)
    ...
    def prepare_new_operation_move_values(self, debit_account, credit_account, name, partner=False):
        ...
        debit, credit, amount_currency, currency_id = self.env['account.move.line']._compute_amount_fields(
            amount, self.currency_id, self.company_currency_id)
