...
class print_picking_barcode_wiz(models.TransientModel):
    _name = 'print.picking.barcode.wiz'
    ...
    def default_get(self, fields_list):
        product_list = []
        res = super(print_picking_barcode_wiz, self).default_get(fields_list)
        product_label_obj = self.env['product.product.label']
        if self._context.get('active_ids'):
            for each_picking in self.env['stock.picking'].browse(self._context.get('active_ids')):
                no_of_product = len(each_picking.move_lines)
                for each_move in each_picking.move_lines:
    ...
    def print_label(self):
        ...
        ctx = dict(context, active_model='print.picking.barcode.wiz', active_ids=ids, active_id=ids[0])
    ...
    def show_label(self):
        ...
        datas = {
                ...
                'model': 'print.picking.barcode.wiz'
            }
        res = {
            'type' : 'ir.actions.report.xml',
            'report_name': 'picking_product_label_print',
            'datas' : datas,
        }
        return self.env['report'].get_action(self, 'picking_product_label_print')
