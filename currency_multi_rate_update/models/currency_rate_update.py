...
class CurrencyRateUpdateService(models.Model):
    ...
    # Mapping of currency rate types
    rate_type_ids = fields.One2many('currency.rate.update.type', 'service_id',
            ...
            try:
                ...
                for curr in self.currency_to_update:
                    if curr.id == main_currency.id:
                        continue
                    for rate_type in self.rate_type_ids:
                        do_create = True
                        for rate in curr.rate_ids.filtered(lambda r: r.company_id.id == self.company_id.id):
                            if rate.name == rate_name and rate_type.currency_rate_type_id == rate.currency_rate_type_id:
                                rate.rate = res[curr.name+(rate_type.service_rate_type or '')]
                                do_create = False
                                break
                        if do_create:
                            vals = {
                                'currency_id': curr.id,
                                'rate': res[curr.name+(rate_type.service_rate_type or '')],
                                'name': rate_name,
                                'currency_rate_type_id': rate_type.currency_rate_type_id
                                                         and rate_type.currency_rate_type_id.id or False,
                                'company_id': self.company_id.id
                            }
                            rate_obj.create(vals)
                            _logger.info(
                                'Updated currency %s %s via service %s',
                                curr.name, rate_type.service_rate_type or '', self.service)
